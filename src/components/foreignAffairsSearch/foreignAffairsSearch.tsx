import { Box, Button, Container, FileUpload, Grid, Input, Pagination, ProgressBar, Select, SpaceBetween, Table, TextFilter } from "@cloudscape-design/components";
import {Header as CloudScapeHeader} from "@cloudscape-design/components";
import React from "react";



export function ForeignAffairsSearch({onSearchClick}) {
  ...
  const handleClearSearch = () => {
    ...
    /></div>
    <div>
      <Button variant="primary" onClick={() => onSearchClick(nameValue, selectedOption)}>搜索</Button>
      </div>
      <div>
      <Button variant="normal" onClick={handleClearSearch}>清空</Button>
      </div>
    </Grid>
    )
}